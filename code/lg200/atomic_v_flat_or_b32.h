// Atomically reads a 32-bit bit pattern from the addressed flat-memory location, computes new = old OR source, and writes new back to the same location. The destination receives old.
...
u32 updated = v_flat_or_b32(src[0], src[1]);
dst[0] = src[0];
dst[1] = updated;
