// Atomically reads a 64-bit bit pattern from the addressed flat-memory location, computes new = old OR source, and writes new back to the same location. The destination receives old.
...
u64 updated = v_flat_or_b64(((u64)src[0] | ((u64)src[0+1] << 32)), ((u64)src[2] | ((u64)src[2+1] << 32)));
dst[0] = src[0];
dst[1] = src[1];
